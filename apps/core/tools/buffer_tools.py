"""
buffer_tools.py — Social media automation via the Buffer API.
Automatically posts to Twitter/X, LinkedIn, Facebook, Instagram.
"""
...
class BufferTools:
    """Automated social media posting via Buffer."""
    ...
    async def get_profiles(self) -> dict[str, Any]:
        """Gets all profiles connected to Buffer."""
        if not self._token:
            return {"success": False, "error": "BUFFER_TOKEN not configured"}
        try:
    ...
    async def post_update(
        ...
        """Publishes an update to all (or some) connected profiles."""
        if not self._token:
            return {"success": False, "error": "BUFFER_TOKEN not configured"}
        try:
            # If no IDs are given, get all profiles
            if not profile_ids:
            ...
            if not profile_ids:
                return {"success": False, "error": "No profiles connected to Buffer"}
            ...
            if res.status_code == 200:
                ...
                logger.info("[BufferTools] %d updates created", len(updates))
                return {
                    "success": True,
    ...
    async def get_pending_updates(self, profile_id: str) -> dict[str, Any]:
        """Gets the posts pending in a profile's queue."""
        if not self._token:
            return {"success": False, "error": "BUFFER_TOKEN not configured"}
        try:
            res = await self._http.get(
    ...
    async def analyze_best_posting_times(self, niche: str, language: str = "es") -> list[str]:
        """Generates the best posting times based on the niche."""
        schedules = {
