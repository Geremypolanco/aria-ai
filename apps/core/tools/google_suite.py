...
class GoogleSuite:
    ...
    async def vision_analyze(
        ...
        if not self._ok():
            return {"success": False, "error": "GOOGLE_API_KEY no configurado"}
        if not features:
            features = ["LABEL_DETECTION","TEXT_DETECTION","OBJECT_LOCALIZATION","WEB_DETECTION","IMAGE_PROPERTIES"]
        try:
            ...
            if res.status_code == 200:
                ...
                if "textAnnotations" in r:
                    result["full_text"] = r["textAnnotations"][0]["description"] if r["textAnnotations"] else ""
                ...
                if "webDetection" in r:
                    web = r["webDetection"]
                    result["web_entities"] = [e["description"] for e in web.get("webEntities", [])[:5]]
                    result["similar_images"] = [p["url"] for p in web.get("fullMatchingImages", [])[:3]]
                # Propiedades de imagen
                if "imagePropertiesAnnotation" in r:
                    colors = r["imagePropertiesAnnotation"].get("dominantColors", {}).get("colors", [])
                ...
                return result
            return {"success": False, "error": f"Vision API HTTP {res.status_code}: {res.text[:200]}"}
        except Exception as exc:
    ...
    async def vision_ocr(self, image_url: str = "", image_bytes: bytes = b"") -> dict[str, Any]:
        ...
        result = await self.vision_analyze(image_url=image_url, image_bytes=image_bytes,
        ...
        try:
            ...
            if isinstance(sentiment_r, dict) and "documentSentiment" in sentiment_r:
                ...
                result["sentiment"] = {
                    ...
                    "label": "positivo" if s.get("score", 0) > 0.1 else "negativo" if s.get("score", 0) < -0.1 else "neutro",
                }
    ...
    async def translate_batch(self, texts: list[str], target: str) -> list[str]:
        ...
        return [r.get("translated", t) if isinstance(r, dict) and r.get("success") else t
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                return {"success": True, "audio_bytes": audio_bytes, "format": "mp3",
                        "language": language_code, "voice": voice_name}
    ...
    async def list_tts_voices(self, language_code: str = "") -> dict[str, Any]:
        ...
        try:
            ...
            res = await self._http.get("https://texttospeech.googleapis.com/v1/voices", params=params)
            if res.status_code == 200:
                voices = res.json().get("voices", [])
                return {
                    "success": True,
                    "count": len(voices),
                    "voices": [{"name": v["name"], "language": v["languageCodes"][0],
                                "gender": v.get("ssmlGender",""), "type": "Neural2" if "Neural" in v["name"] else "Standard"}
                               for v in voices[:30]],
                }
            return {"success": False, "error": f"HTTP {res.status_code}"}
        except Exception as exc:
    ...
    async def speech_to_text(
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                confidence = results[0].get("alternatives", [{}])[0].get("confidence", 0) if results else 0
                return {"success": True, "transcript": transcript, "confidence": round(confidence, 3), "language": language}
    ...
    async def youtube_search(self, query: str, max_results: int = 20, order: str = "relevance",
                              video_type: str = "video", language: str = "") -> dict[str, Any]:
        """Búsqueda completa en YouTube."""
        if not self._ok():
            return {"success": False, "error": "GOOGLE_API_KEY no configurado"}
        try:
            params = {"part":"snippet","q":query,"type":video_type,"maxResults":min(max_results,50),
                      "order":order,"key":self._key,"safeSearch":"moderate"}
            if language:
            ...
            if res.status_code == 200:
                items = res.json().get("items", [])
                return {
                    "success": True, "query": query, "count": len(items),
                    "results": [
    ...
    async def youtube_video_details(self, video_ids: list[str]) -> dict[str, Any]:
        ...
        if not self._ok():
            return {"success": False, "error": "GOOGLE_API_KEY no configurado"}
        try:
            res = await self._http.get(f"{BASE}/youtube/v3/videos", params={
    ...
    async def youtube_channel_details(self, channel_id: str = "", username: str = "") -> dict[str, Any]:
        """Detalles completos de un canal de YouTube."""
        if not self._ok():
            return {"success": False, "error": "GOOGLE_API_KEY no configurado"}
        try:
            params = {"part":"snippet,statistics,brandingSettings,contentDetails","key":self._key}
            if channel_id:
    ...
    async def youtube_comments(self, video_id: str, max_results: int = 20) -> dict[str, Any]:
        ...
        if not self._ok():
            return {"success": False, "error": "GOOGLE_API_KEY no configurado"}
        try:
            res = await self._http.get(f"{BASE}/youtube/v3/commentThreads", params={
                "part":"snippet","videoId":video_id,"maxResults":min(max_results,100),
                "order":"relevance","key":self._key,
            })
            if res.status_code == 200:
                items = res.json().get("items", [])
                return {
    ...
    async def youtube_search_captions(self, video_id: str) -> dict[str, Any]:
        ...
        if not self._ok():
            return {"success": False, "error": "GOOGLE_API_KEY no configurado"}
        try:
            res = await self._http.get(f"{BASE}/youtube/v3/captions", params={
                "part":"snippet","videoId":video_id,"key":self._key,
            })
            if res.status_code == 200:
                items = res.json().get("items", [])
                return {"success": True, "captions": [{"id":i["id"],"language":i["snippet"]["language"],"name":i["snippet"]["name"]} for i in items]}
            return {"success": False, "error": f"HTTP {res.status_code}"}
        except Exception as exc:
            return {"success": False, "error": str(exc)}
    ...
    async def knowledge_graph_search(self, query: str, languages: list[str] | None = None, limit: int = 5) -> dict[str, Any]:
        ...
        try:
            ...
            res = await self._http.get("https://kgsearch.googleapis.com/v1/entities:search", params=params)
            if res.status_code == 200:
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                return {
                    ...
                    "opportunities": [
                        {"title": audits[k].get("title",""), "savings": audits[k].get("displayValue","")}
                        for k in audits if audits[k].get("details",{}).get("type") == "opportunity"
    ...
    async def books_search(self, query: str, max_results: int = 10, language: str = "") -> dict[str, Any]:
        """Busca libros en Google Books (útil para investigación de nichos)."""
        if not self._ok():
            return {"success": False, "error": "GOOGLE_API_KEY no configurado"}
        try:
            params = {"q":query,"maxResults":min(max_results,40),"key":self._key,"printType":"books","orderBy":"relevance"}
            if language:
    ...
    async def full_market_research(self, niche: str, language: str = "es") -> dict[str, Any]:
        ...
        youtube_task  = self.youtube_search(f"how to make money with {niche}", max_results=5, order="viewCount")
        ...
        web_snippets = " ".join([r.get("snippet","") for r in (web.get("results",[]) if isinstance(web,dict) else [])[:5]])
