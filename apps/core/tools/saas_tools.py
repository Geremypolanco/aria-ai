...
class NotionTools:
    ...
    async def list_pages(self) -> dict[str, Any]:
        if not self._ok(): return {"success": False, "error": "NOTION_TOKEN no configurado"}
        try:
            res = await self._http.post("https://api.notion.com/v1/search", headers=self._headers, json={"filter": {"property": "object", "value": "page"}})
            if res.status_code == 200:
                pages = res.json().get("results", [])
                return {"success": True, "pages": [{"id": p["id"], "url": p.get("url"), "title": p.get("properties", {}).get("title", {}).get("title", [{}])[0].get("plain_text", "Sin título")} for p in pages]}
    ...
    async def create_page(self, parent_id: str, title: str, content: str = "") -> dict[str, Any]:
        ...
        try:
            ...
            res = await self._http.post("https://api.notion.com/v1/pages", headers=self._headers, json=body)
            if res.status_code == 200:
...
class VercelTools:
    ...
    async def list_projects(self) -> dict[str, Any]:
        if not self._ok(): return {"success": False, "error": "VERCEL_TOKEN no configurado"}
        try:
            res = await self._http.get("https://api.vercel.com/v9/projects", headers=self._headers)
            if res.status_code == 200:
                projects = res.json().get("projects", [])
                return {"success": True, "projects": [{"id": p["id"], "name": p["name"], "url": p.get("link")} for p in projects]}
            return {"success": False, "error": f"HTTP {res.status_code}"}
        except Exception as exc:
            return {"success": False, "error": str(exc)}
    ...
    async def get_deployment_status(self, project_id: str) -> dict[str, Any]:
        if not self._ok(): return {"success": False, "error": "VERCEL_TOKEN no configurado"}
        try:
            res = await self._http.get(f"https://api.vercel.com/v6/deployments", headers=self._headers, params={"projectId": project_id, "limit": 1})
            if res.status_code == 200:
                deps = res.json().get("deployments", [])
                if not deps: return {"success": False, "error": "No hay despliegues"}
