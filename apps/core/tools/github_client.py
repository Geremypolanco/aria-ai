...
class AriaGitHubClient:
    ...
    async def get_file(self, owner: str, repo: str, path: str, branch: str = "main") -> dict:
        ...
        data = await self._get(
            f"/repos/{owner}/{repo}/contents/{path.lstrip('/')}", {"ref": branch}
        )
        if "error" in data:
            return data
    ...
    async def list_commits(
        self, owner: str, repo: str, branch: str = "main", per_page: int = 10
    ) -> list | dict:
    ...
    async def create_issue(
        self, owner: str, repo: str, title: str, body: str = "", labels: list = None
    ) -> dict:
    ...
    async def search_repos(self, query: str, per_page: int = 5) -> dict:
        return await self._get(
            "/search/repositories", {"q": query, "per_page": per_page, "sort": "stars"}
        )
    ...
    async def self_commit(
        self, path: str, new_content: str, commit_message: str, branch: str = "main"
    ) -> str:
...
_client: AriaGitHubClient | None = None
