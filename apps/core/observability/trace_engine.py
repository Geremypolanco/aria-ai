...
class TraceEngine:
    ...
    async def trace_action(self,
                          action_type: str,
                          input_data: Dict[str, Any],
                          output_data: Dict[str, Any],
                          duration_ms: float,
                          success: bool,
                          roi: float = 0.0) -> Dict[str, Any]:
        ...
        logger.info(f"[Trace] {action_type}: {'✓' if success else '✗'} (ROI: ${roi}, {duration_ms}ms)")
        return trace
        ...
        return [
            {
                ...
                "failure_rate": (len(failures) / max(1, self.metrics.get(atype, {}).get("count", 1))) * 100,
