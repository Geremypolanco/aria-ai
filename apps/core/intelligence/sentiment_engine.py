    ...
    def update_sentiment(self, event: str, impact: Dict[str, float]):
        ...
        for sentiment, change in impact.items():
            if sentiment in self.current_sentiment:
                self.current_sentiment[sentiment] = max(0.0, min(1.0, self.current_sentiment[sentiment] + change))
        logger.info(f"Sentimiento actualizado por evento '{event}'. Nuevo estado: {self.current_sentiment}")
