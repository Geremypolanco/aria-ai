...
class AriaDatabase:
    ...
    async def log(
        self, level: str, message: str, agent: str = "system", metadata: dict | None = None
    ):
    ...
    async def log_info(self, message: str, agent: str = "system", metadata: dict = None):
        if metadata is None:
            metadata = {}
        await self.log("INFO", message, agent, metadata)

    async def log_error(self, message: str, agent: str = "system", metadata: dict = None):
        if metadata is None:
            metadata = {}
        await self.log("ERROR", message, agent, metadata)

    async def log_success(self, message: str, agent: str = "system", metadata: dict = None):
        if metadata is None:
            metadata = {}
        await self.log("SUCCESS", message, agent, metadata)

    async def log_money(self, message: str, agent: str = "cfo", metadata: dict = None):
        if metadata is None:
            metadata = {}
        await self.log("REVENUE", message, agent, metadata)

    async def get_recent_logs(self, limit: int = 10, level: str | None = None) -> list:
        try:
            q = (
    ...
    async def resolve_approval(self, approval_id: str, decision: str) -> bool:
        ...
        try:
            ...
            self._client.table("approvals").update(
                {"status": decision, "decided_at": datetime.now(UTC).isoformat()}
    ...
    async def get_revenue_this_month(self) -> float:
        try:
            ...
            start = datetime(now.year, now.month, 1, tzinfo=UTC).isoformat()
            result = (
                self._client.table("revenue").select("amount").gte("created_at", start).execute()
            )
            return sum(float(r.get("amount", 0)) for r in (result.data or []))
        except Exception:
            return 0.0
    ...
    async def update_product_stats(
        self, product_id: str, sales_count: int, revenue_usd: float
    ) -> bool:
        try:
    ...
    async def save_market_intelligence(
        ...
        try:
            result = (
                self._client.table("market_intelligence")
                .insert(
                    {
                        "niche": niche,
                        "language": language,
    ...
    async def save_cycle(
        ...
        try:
            ...
            result = (
                self._client.table("autonomous_cycles")
                .insert(
                    {
    ...
    async def get_cycle_stats(self) -> dict:
        try:
            ...
            return {
                ...
                "avg_duration_ms": int(
                    sum(c.get("duration_ms", 0) for c in cycles) / max(len(cycles), 1)
                ),
    ...
    async def save_marketing_campaign(
        ...
        try:
            ...
            result = (
                self._client.table("marketing_campaigns")
                .insert(
                    {
                        "name": name,
                        "platform": platform,
...
_db_instance: AriaDatabase | None = None
