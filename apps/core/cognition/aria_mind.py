...
- discover_leads    → finds B2B leads in a niche via real web search only. Returns an honest empty result when the source is down — never invents companies or pads with examples. Args: {{"niche": "...", "count": 10, "location": "US"}}
...
- `find leads in [niche]` — real web search only; honest "no leads found" when the source is down
    ...
    async def handle(
        ...
        try:
            ...
            if img_prompt:
                ...
                # obs is raw tool output — on failure it can be an exception message or
