...
class CommandRouter:
    ...
    async def _cmd_run(self, args: str, session_id: str) -> str:
        ...
        await _notify(session_id, f"🚀 Iniciando pipeline para: _{args[:80]}_\n⏳ Esto puede tomar 30-90 segundos...")
        ...
        stage_icon = {"complete": "✅", "failed": "❌", "execute": "⚡"}.get(s.get("stage", ""), "🔄")
    ...
    async def _cmd_goals(self, args: str, session_id: str) -> str:
        ...
        if not goals:
            return "🎯 No hay metas activas. Usa `/run [misión]` para que ARIA defina sus objetivos."
        ...
        for i, g in enumerate(goals[:10], 1):
            ...
            lines.append(
                ...
                f"   Prioridad: {priority}/10"
                + (f" | {progress[:80]}" if progress else "")
            )
