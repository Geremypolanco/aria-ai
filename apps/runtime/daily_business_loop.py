...
class DailyBusinessLoop:
    ...
    async def _distribution_op(self, name: str) -> dict:
        if "TikTok" in name or "Reels" in name:
            ...
            return {
                ...
                "titles": [s.topic for s in scripts],
            }
    ...
    async def _acquisition_op(self, name: str) -> dict:
        if "leads" in name.lower() or "Discover" in name:
            ...
            return {
                ...
                "niches": list({l.niche for l in leads}),
            }
        ...
        customer_dicts = [
            {
                ...
                "churn_risk": (
                    c.churn_risk.value if hasattr(c.churn_risk, "value") else str(c.churn_risk)
                ),
            }
    ...
    async def run(self, max_ops: int = len(ALL_OPS)) -> DailyBusinessReport:
        ...
        completed = failed = 0
        direct_count = content_count = leads_count = outreach_count = shopify_count = (
            funnel_count
        ) = 0
        ...
        try:
            ...
            if resp.success:
                ...
                report.top_insight = (
                    lines[0]
                    if lines
                    else "Execute consistently — compound growth rewards daily action."
                )
                report.tomorrow_priority = (
                    lines[1] if len(lines) > 1 else "Increase outreach volume by 50%."
                )
        except Exception:
            pass
        ...
        if not report.tomorrow_priority:
            report.tomorrow_priority = (
                "Prioritize content distribution and outreach for maximum reach."
            )
    ...
    async def generate_status_report(self) -> dict:
        ...
        return {
            ...
            "last_report_date": (
                self._reports[-1].get("date", "never") if self._reports else "never"
            ),
        }
...
# ── Singleton ─────────────────────────────────────────────────────────────────
_instance: DailyBusinessLoop | None = None
