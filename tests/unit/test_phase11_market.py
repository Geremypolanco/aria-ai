"""Phase 11 tests — Market (PricingIntelligence, LinkingOptimizer)."""
...
class TestPricingIntelligence:
    ...
    def intel(self):
        with patch(
            "apps.market.pricing.pricing_intelligence.get_cache", return_value=_mock_cache()
        ):
            with patch(
                "apps.market.pricing.pricing_intelligence.get_ai_client",
    ...
    async def test_analyze_pricing_with_competitor_data(self, intel):
        ...
        pp = await intel.analyze_pricing(
            "AI Writing Tool", "software", 69.0, competitor_data=competitors
        )
    ...
    async def test_positioning_premium_when_overpriced(self, intel):
        ...
        pp = await intel.analyze_pricing(
            "Luxury Product", "luxury", 80.0, competitor_data=competitors
        )
    ...
    async def test_positioning_budget_when_underpriced(self, intel):
        ...
        pp = await intel.analyze_pricing(
            "Budget Product", "general", 50.0, competitor_data=competitors
        )
...
class TestLinkingOptimizer:
    ...
    def optimizer(self):
        with patch(
            "apps.content.internal_linking.linking_optimizer.get_cache", return_value=_mock_cache()
        ):
            with patch(
                "apps.content.internal_linking.linking_optimizer.get_ai_client",
                return_value=_mock_ai(
                    "Pillar: Complete Guide to Fitness\nCluster: Nutrition, Workouts, Recovery"
                ),
            ):
