...
class TestLinkedInOutreach:
    ...
    def outreach(self):
        with patch(
            "apps.acquisition.linkedin.linkedin_outreach.get_cache", return_value=_mock_cache()
        ):
            with patch(
                "apps.acquisition.linkedin.linkedin_outreach.get_ai_client",
                return_value=_mock_ai(
                    "0.8 This prospect is highly relevant as they manage marketing budgets."
                ),
            ):
    ...
    def test_update_prospect_status(self, outreach):
        ...
        p = asyncio.run(outreach.add_prospect("Iris", "Manager", "Corp", "retail"))
...
class TestUpworkBidder:
    ...
    def bidder(self):
        with patch("apps.acquisition.upwork.upwork_bidder.get_cache", return_value=_mock_cache()):
            with patch(
                "apps.acquisition.upwork.upwork_bidder.get_ai_client",
                return_value=_mock_ai(
                    "fit_score: 0.85 — strong match for this project budget $500"
                ),
            ):
    ...
    async def test_evaluate_job_sets_bid_price(self, bidder):
        job = await bidder.evaluate_job(
            "Content Writer", "Blog posts", 100.0, 300.0, ["writing"], ["writing", "seo"]
        )
...
class TestFiverrOptimizer:
    ...
    def optimizer(self):
        with patch(
            "apps.acquisition.fiverr.fiverr_optimizer.get_cache", return_value=_mock_cache()
        ):
            with patch(
                "apps.acquisition.fiverr.fiverr_optimizer.get_ai_client",
                return_value=_mock_ai(
                    "I will create professional AI content for your fitness brand"
                ),
            ):
...
class TestOutreachSequencer:
    ...
    def sequencer(self):
        with patch(
            "apps.acquisition.outreach.outreach_sequencer.get_cache", return_value=_mock_cache()
        ):
            with patch(
                "apps.acquisition.outreach.outreach_sequencer.get_ai_client",
                return_value=_mock_ai(
    ...
    async def test_enroll_contact_returns_contact(self, sequencer):
        ...
        contact = await sequencer.enroll_contact(
            "John Doe", "john@corp.com", "Corp", seq.sequence_id
        )
    ...
    async def test_personalize_step_returns_dict(self, sequencer):
        seq = await sequencer.create_sequence(
            "Personalise Test", "linkedin", "founder", "meeting", steps=3
        )
