...
DifferentiationEngine, CreativeIdentityManager, GoalManager, RevenueLoopEngine,
SelfDirector, OperationsManager, ExecutiveDashboard, CashflowEngine,
BusinessAnalytics.
"""
...
class TestGoalManager:
    @pytest.fixture
    def manager(self):
        with patch("apps.autonomy.goals.goal_manager.get_cache", return_value=_mock_cache()):
            from apps.autonomy.goals.goal_manager import GoalManager
            return GoalManager()

    @pytest.mark.asyncio
    async def test_add_goal(self, manager):
        from apps.autonomy.goals.goal_manager import GoalHorizon
        goal = await manager.add_goal(
            title="Reach $5K Revenue",
            horizon=GoalHorizon.MONTHLY,
            metrics_data=[{"name": "revenue_usd", "target": 5000.0, "current": 0.0, "unit": "USD"}],
            priority=8,
        )
        assert goal.goal_id
        assert goal.title == "Reach $5K Revenue"

    @pytest.mark.asyncio
    async def test_update_metric_achieves_goal(self, manager):
        from apps.autonomy.goals.goal_manager import GoalHorizon, GoalStatus
        goal = await manager.add_goal(
            "Test Goal", GoalHorizon.WEEKLY,
            [{"name": "revenue_usd", "target": 100.0, "current": 0.0, "unit": "USD"}],
        )
        updated = await manager.update_metric(goal.goal_id, "revenue_usd", 150.0)
        assert updated is not None
        assert updated.status == GoalStatus.ACHIEVED

    @pytest.mark.asyncio
    async def test_get_active_goals(self, manager):
        from apps.autonomy.goals.goal_manager import GoalHorizon
        await manager.add_goal("G1", GoalHorizon.DAILY, [{"name": "m1", "target": 10.0, "current": 0.0, "unit": "count"}])
        await manager.add_goal("G2", GoalHorizon.WEEKLY, [{"name": "m2", "target": 5.0, "current": 0.0, "unit": "count"}])
        active = await manager.get_active_goals()
        assert len(active) >= 2

    @pytest.mark.asyncio
    async def test_generate_default_goals(self, manager):
        defaults = await manager.generate_default_goals()
        assert len(defaults) >= 1

    @pytest.mark.asyncio
    async def test_goal_dashboard(self, manager):
        from apps.autonomy.goals.goal_manager import GoalHorizon
        await manager.add_goal("Dash Goal", GoalHorizon.QUARTERLY,
                               [{"name": "revenue", "target": 10000.0, "current": 1000.0, "unit": "USD"}])
        dash = await manager.goal_dashboard()
...
# ══════════════════════════════════════════════════════════════════════════════
# 20. REVENUE LOOP ENGINE
# ══════════════════════════════════════════════════════════════════════════════

class TestRevenueLoopEngine:
    @pytest.fixture
    def engine(self):
        with patch("apps.autonomy.revenue_loops.revenue_loop_engine.get_cache", return_value=_mock_cache()):
            from apps.autonomy.revenue_loops.revenue_loop_engine import RevenueLoopEngine
            return RevenueLoopEngine()

    @pytest.mark.asyncio
    async def test_create_loop(self, engine):
        loop = await engine.create_loop("Email Revenue", "email", "Weekly email monetization")
        assert loop.loop_id
        assert loop.channel == "email"
        assert loop.active

    @pytest.mark.asyncio
    async def test_run_loop(self, engine):
        from apps.autonomy.revenue_loops.revenue_loop_engine import LoopPhase
        loop = await engine.create_loop("Shopify Opt", "shopify")
        execution = await engine.run_loop(loop.loop_id)
        assert execution.loop_id == loop.loop_id
        assert isinstance(execution.actions_taken, list)

    @pytest.mark.asyncio
    async def test_default_loops(self, engine):
        loops = await engine.default_loops()
        assert len(loops) >= 2

    @pytest.mark.asyncio
    async def test_loop_analytics(self, engine):
        await engine.create_loop("Test Loop", "content")
        analytics = await engine.loop_analytics()
        assert "total_loops" in analytics
        assert "total_revenue_usd" in analytics

    @pytest.mark.asyncio
    async def test_due_loops(self, engine):
        loop = await engine.create_loop("Past Due", "email")
        # Force next_run_ts to past
        loop_dict = engine._loops[loop.loop_id]
        loop_dict["next_run_ts"] = time.time() - 3600
        engine._loops[loop.loop_id] = loop_dict
        due = await engine.due_loops()
        assert len(due) >= 1
...
class TestSelfDirector:
    @pytest.fixture
    def director(self):
        with patch("apps.autonomy.self_direction.self_director.get_cache", return_value=_mock_cache()):
            with patch("apps.autonomy.self_direction.self_director.get_ai_client", return_value=_mock_ai()):
                from apps.autonomy.self_direction.self_director import SelfDirector
                return SelfDirector()

    @pytest.mark.asyncio
    async def test_generate_directive_scale(self, director):
        from apps.autonomy.self_direction.self_director import DirectiveType
        directive = await director.generate_directive({
            "revenue_trend": "up",
            "top_performing_channel": "email",
            "biggest_bottleneck": "traffic",
            "available_budget_usd": 2000.0,
        })
        assert directive.directive_id
        assert directive.type == DirectiveType.SCALE
        assert len(directive.actions) >= 1

    @pytest.mark.asyncio
    async def test_generate_directive_pivot(self, director):
        from apps.autonomy.self_direction.self_director import DirectiveType
        directive = await director.generate_directive({
            "revenue_trend": "down",
            "top_performing_channel": "organic",
            "biggest_bottleneck": "conversion",
            "available_budget_usd": 500.0,
        })
        assert directive.type == DirectiveType.PIVOT

    @pytest.mark.asyncio
    async def test_pending_directives(self, director):
        await director.generate_directive({
            "revenue_trend": "flat", "top_performing_channel": "social",
            "biggest_bottleneck": "retention", "available_budget_usd": 300.0,
        })
        pending = await director.pending_directives()
        assert len(pending) >= 1
        assert all(not d.executed for d in pending)

    @pytest.mark.asyncio
    async def test_mark_executed(self, director):
        directive = await director.generate_directive({
            "revenue_trend": "up", "top_performing_channel": "email",
            "biggest_bottleneck": "none", "available_budget_usd": 1500.0,
        })
        result = await director.mark_executed(directive.directive_id)
        assert result is not None
        assert result.executed

    @pytest.mark.asyncio
    async def test_autonomous_cycle(self, director):
        result = await director.autonomous_cycle({
            "revenue_trend": "up", "top_performing_channel": "shopify",
            "biggest_bottleneck": "traffic", "available_budget_usd": 800.0,
        })
        assert "directive" in result
        assert "next_actions" in result

    def test_summary(self, director):
        assert isinstance(director.summary(), dict)
