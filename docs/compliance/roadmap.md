...
| Dependency/vulnerability scanning + static analysis in CI | In place | CI "Security Audit" job: both `pip-audit` and `bandit` are blocking merge gates (each with a documented, minimal ignore/nosec list — see `ci.yml` and inline `# nosec` comments) |
